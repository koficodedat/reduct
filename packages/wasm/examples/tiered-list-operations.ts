/**
 * Example demonstrating the tiered optimization for list operations
 */
...
console.log('Tiered List Operations Example');
console.log('=============================');
...
for (const size of sizes) {
  ...
  // Determine the tier
  const tier = listAccelerator.determineTier(array);
  
  // Map the array and measure performance
  console.time(`Map ${size} elements (${tier})`);
  const mapped = listAccelerator.map(array, x => x * 2);
...
for (const size of sizes) {
  ...
  const filtered = listAccelerator.filter(array, x => x > 0.5);
...
for (const size of sizes) {
  ...
  const sum = listAccelerator.reduce(array, (acc, x) => acc + x, 0);
...
for (const size of sizes) {
  ...
  const sorted = listAccelerator.sort(array);
  console.timeEnd(`Sort ${size} elements (${tier})`);
  
  console.log(`Array size: ${size}, Tier: ${tier}, Result length: ${sorted.length}`);
}

// Test mapFilter operation
console.log('\nTesting mapFilter operation with different array sizes:');
console.log('---------------------------------------------------');
...
for (const size of sizes) {
  ...
  // Map and filter the array and measure performance
  console.time(`MapFilter ${size} elements (${tier})`);
  const result = listAccelerator.mapFilter(array, x => x * 2, x => x > 1);
  console.timeEnd(`MapFilter ${size} elements (${tier})`);
  
  console.log(`Array size: ${size}, Tier: ${tier}, Result length: ${result.length}`);
}

// Test mapReduce operation
console.log('\nTesting mapReduce operation with different array sizes:');
console.log('---------------------------------------------------');
...
for (const size of sizes) {
  ...
  // Map and reduce the array and measure performance
  console.time(`MapReduce ${size} elements (${tier})`);
  const result = listAccelerator.mapReduce(array, x => x * 2, (acc, x) => acc + x, 0);
  console.timeEnd(`MapReduce ${size} elements (${tier})`);
  
  console.log(`Array size: ${size}, Tier: ${tier}, Result: ${result.toFixed(2)}`);
}

// Test filterReduce operation
console.log('\nTesting filterReduce operation with different array sizes:');
console.log('-----------------------------------------------------');
...
for (const size of sizes) {
  ...
  // Filter and reduce the array and measure performance
  console.time(`FilterReduce ${size} elements (${tier})`);
  const result = listAccelerator.filterReduce(array, x => x > 0.5, (acc, x) => acc + x, 0);
  console.timeEnd(`FilterReduce ${size} elements (${tier})`);
  
  console.log(`Array size: ${size}, Tier: ${tier}, Result: ${result.toFixed(2)}`);
}

// Test mapFilterReduce operation
console.log('\nTesting mapFilterReduce operation with different array sizes:');
console.log('----------------------------------------------------------');
...
for (const size of sizes) {
  ...
  // Map, filter, and reduce the array and measure performance
  console.time(`MapFilterReduce ${size} elements (${tier})`);
  const result = listAccelerator.mapFilterReduce(array, x => x * 2, x => x > 1, (acc, x) => acc + x, 0);
  console.timeEnd(`MapFilterReduce ${size} elements (${tier})`);
...
console.log('---------------------');
const stats = listAccelerator.getPerformanceStats();
...
for (const tier of tiers) {
  ...
  // Create a custom list accelerator with fixed tier
  const fixedTierAccelerator = new ListAccelerator();
  ...
  // Map-filter the array and measure performance
  console.time(`MapFilter with ${tier}`);
  fixedTierAccelerator.mapFilter(largeArray, x => x * 2, x => x > 1);
  console.timeEnd(`MapFilter with ${tier}`);
}

console.log('\nTiered list operations example completed.');
